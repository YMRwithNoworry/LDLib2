...
public final class LDLibExtraCodecs {
    ...
    public final static Codec<Tag> TAG = Codec.PASSTHROUGH.xmap(
            dynamic -> dynamic.convert(NbtOps.INSTANCE).getValue(),
            tag -> new Dynamic<>(NbtOps.INSTANCE, tag)
    );

    public final static Codec<Recipe<?>> RECIPE_HOLDER_ID = ResourceLocation.CODEC.flatXmap(id -> {
    ...
    }, recipe -> DataResult.error(() -> "Recipe id encoding is not available on 1.20.1 without its manager key"));
    ...
    public static final Codec<ItemStack> ITEM_STACK = new Codec<>() {
        ...
        public <T> DataResult<Pair<ItemStack, T>> decode(DynamicOps<T> ops, T input) {
            ...
            if (tag instanceof CompoundTag compoundTag) {
                return DataResult.success(Pair.of(ItemStack.of(compoundTag), input));
            }
            return DataResult.error(() -> "Expected item stack compound tag");
        }
    ...
    public static final Codec<FluidStack> FLUID_STACK = new Codec<>() {
        @Override
        public <T> DataResult<Pair<FluidStack, T>> decode(DynamicOps<T> ops, T input) {
            var tag = ops.convertTo(NbtOps.INSTANCE, input);
            if (tag instanceof CompoundTag compoundTag) {
                return DataResult.success(Pair.of(FluidStack.parseOptional(Platform.getFrozenRegistry(), compoundTag), input));
            }
            return DataResult.error(() -> "Expected fluid stack compound tag");
        }

        @Override
        public <T> DataResult<T> encode(FluidStack input, DynamicOps<T> ops, T prefix) {
            return DataResult.success(NbtOps.INSTANCE.convertTo(ops, input == null || input.getFluid() == Fluids.EMPTY ? new CompoundTag() : input.save(Platform.getFrozenRegistry())));
        }
    };
