...
public final class PersistedParser {
    ...
    public static <T> MapCodec<T> createMapCodec(Supplier<T> creator) {
        return new MapCodec<>() {
            private final Codec<T> codec = createCodec(creator);

            @Override
            public <O> RecordBuilder<O> encode(T input, DynamicOps<O> ops, RecordBuilder<O> prefix) {
                codec.encodeStart(ops, input).result().flatMap(data -> ops.getMap(data).result()).ifPresent(map ->
                        map.entries().forEachOrdered(entry -> prefix.add(entry.getFirst(), entry.getSecond())));
                return prefix;
            }
                        ...
                        if (value instanceof INBTSerializable<?> serializable) {
                            ...
                            if (subFlatten) {
                                ...
                                if (mapResult.result().isPresent()) {
                                    mapResult.getOrThrow(false, message -> {}).entries().forEachOrdered(entry ->
                            ...
                            if (!subFlatten) {
                                data = builder.build(op.empty()).getOrThrow(false, message -> {});
                            }
                        }
                    }
    ...
    private static void writeStreamBuffInternal(boolean root, RegistryFriendlyByteBuf buf, Class<?> clazz, Object object, HolderLookup.Provider provider) {
        ...
        if (root && object instanceof IPersistedSerializable serializable) {
            ...
            if (additional != null && additional != EndTag.INSTANCE) {
                buf.writeBoolean(true);
                buf.writeNbt(additional instanceof CompoundTag compoundTag ? compoundTag : new CompoundTag());
            } else {
                buf.writeBoolean(false);
            }
            serializable.afterSerialize();
            ...
            if (persistent.map(Configurable::subConfigurable, Persisted::subPersisted)) {
                try {
                    ...
                    if (buf.readBoolean() && value != null) {
                        if (value instanceof INBTSerializable serializable) {
                            serializable.deserializeNBT(provider, buf.readNbt());
                        } else {
