...
public class CustomDirectAccessor<TYPE> implements IDirectAccessor<TYPE>, IMarkFunction<TYPE, Object> {
    ...
    public <T> T readDirectVar(DynamicOps<T> op, IVar<TYPE> var) {
        return codec.encodeStart(op, var.value()).getOrThrow(false, LDLib2.LOGGER::error);
    }

    @Override
    public <T> void writeDirectVar(DynamicOps<T> op, IVar<TYPE> var, T payload) {
        var.set(codec.parse(op, payload).getOrThrow(false, LDLib2.LOGGER::error));
    }
    ...
    public static class Builder<TYPE> {
        ...
        /**
         * Use the codec to mark the value.
         */
        public Builder<TYPE> codecMark() {
            this.markFunction = new IMarkFunction.Simple<>(
                    value -> codec.encodeStart(JsonOps.INSTANCE, value).getOrThrow(false, LDLib2.LOGGER::error),
                    (mark, value) -> !Objects.equals(mark, codec.encodeStart(JsonOps.INSTANCE, value).getOrThrow(false, LDLib2.LOGGER::error)));
            return this;
        }
