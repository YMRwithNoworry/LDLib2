    ...
    public final void readSyncToStream(RegistryFriendlyByteBuf buffer) {
        if (isReadOnlyManaged()) {
            ...
            if (value == null) {
                ...
                var uid = getReadOnlyVar().getManagedVar().serializeUid(value);
                buffer.writeNbt(uid instanceof CompoundTag compoundTag ? compoundTag : new CompoundTag());
    ...
    public final <T> T readInitialSync(DynamicOps<T> op) {
        if (isReadOnlyManaged()) {
            ...
            return op.mapBuilder()
                    ...
                    .build(op.empty()).getOrThrow(false, message -> {});
        } else {
