...
public class ModelFactory {
    ...
    private static Field modelBakeryField;

    public static ModelBakery getModelBakery() {
        try {
            if (modelBakeryField == null) {
                for (var field : Minecraft.getInstance().getModelManager().getClass().getDeclaredFields()) {
                    if (ModelBakery.class.isAssignableFrom(field.getType())) {
                        field.setAccessible(true);
                        modelBakeryField = field;
                        break;
                    }
                }
            }
            if (modelBakeryField != null) {
                return (ModelBakery) modelBakeryField.get(Minecraft.getInstance().getModelManager());
            }
        } catch (ReflectiveOperationException ignored) {
        }
        throw new IllegalStateException("Unable to access ModelBakery from ModelManager");
    }
        ...
        return new ModelBaker() {
            ...
            public BakedModel bake(ResourceLocation location, ModelState transform) {
                ...
                if (unbakedmodel instanceof BlockModel blockmodel && blockmodel.getRootModel() == ModelBakery.GENERATION_MARKER) {
                    return ITEM_MODEL_GENERATOR.generateBlockModel(Material::sprite, blockmodel)
                            .bake(this, blockmodel, Material::sprite, transform, location, false);
                }
