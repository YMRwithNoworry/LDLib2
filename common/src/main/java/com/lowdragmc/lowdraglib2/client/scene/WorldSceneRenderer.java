...
public abstract class WorldSceneRenderer {
    ...
    private void cancelCompile() {
        ...
        if (syncCompileState != null && syncCompileState.currentBuffer != null) {
            try {
                var leftover = syncCompileState.currentBuffer.endOrDiscardIfEmpty();
                if (leftover != null) leftover.release();
    ...
    protected void setupCamera(PositionedRect viewport) {
        ...
        posesStack.last().pose().lookAt(eyePos.x(), eyePos.y(), eyePos.z(), lookAt.x(), lookAt.y(), lookAt.z(), worldUp.x(), worldUp.y(), worldUp.z());
...
//        RenderSystem.depthMask(false);
        ...
        thread = new Thread(() -> {
            ...
            try { // render the blocks in each layer
                ...
                for (int i = 0; i < layers.size(); i++) {
                    ...
                    var data = buffer.endOrDiscardIfEmpty();
                    if (data == null) {
    ...
    private void finalizeSyncLayer(SyncCompileState s) {
        ...
        var data = s.currentBuffer.endOrDiscardIfEmpty();
        if (data == null) {
    ...
    public static class VertexConsumerWrapper implements VertexConsumer {
        ...
        public void defaultColor(int red, int green, int blue, int alpha) {
            builder.defaultColor(red, green, blue, alpha);
        }
