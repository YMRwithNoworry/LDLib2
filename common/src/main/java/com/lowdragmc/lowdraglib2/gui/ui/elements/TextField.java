...
public class TextField extends BindableUIElement<String> {
    ...
    public TextField setResourceLocationOnly() {
        ...
        setCharValidator(chr -> chr == ':' || chr == '_' || chr == '-' || chr == '.' || chr == '/' || (chr >= 'a' && chr <= 'z') || (chr >= '0' && chr <= '9'));
    ...
    protected void onCharTyped(UIEvent event) {
        ...
        if (SharedConstants.isAllowedChatCharacter((char) event.codePoint) && charValidator.test(event.codePoint)) {
