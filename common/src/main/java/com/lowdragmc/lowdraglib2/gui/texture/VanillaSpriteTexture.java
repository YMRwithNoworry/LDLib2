...
public class VanillaSpriteTexture extends TransformTexture {
    ...
    @Getter
    @Setter
    private ResourceLocation sprite = new ResourceLocation("minecraft", "toast/recipe_book");
    ...
    private SearchComponentConfigurator.ISearchConfigurator<ResourceLocation> searchSprites() {
        return new SearchComponentConfigurator.ISearchConfigurator<>() {
            ...
            public ResourceLocation defaultValue() {
                return new ResourceLocation("minecraft", "toast/recipe_book");
            }

            @Override
            public void search(String word, IResultHandler<ResourceLocation> searchHandler) {
                var lowerWord = word.toLowerCase();
                if (sprite != null && sprite.toString().toLowerCase().contains(lowerWord)) {
                    searchHandler.acceptResult(sprite);
                } else if (MissingTextureAtlasSprite.getLocation().toString().toLowerCase().contains(lowerWord)) {
                    searchHandler.acceptResult(MissingTextureAtlasSprite.getLocation());
                }
            }
